...
Au début, cette équation peut sembler compliquée et arbitraire, mais elle a en réalité une forme très simple, que nous pouvons visualiser si nous tracons la valeur de $$\sigma(z)$$ en fonction de l'entrée $$z$$.
...
Les neurones sigmoïdes ont été la base de la plupart des réseaux neuronaux pendant des décennies, mais ces dernières années, ils ont devenus obsolétes. La raison de ceci sera expliquée plus en détail plus tard, mais en résumé, ils rendent difficiles les réseaux de neurones qui ont beaucoup de couches à former en raison du [problème du gradient de fuite](https://en.wikipedia.org/wiki/Vanishing_gradient_problem). Au lieu de cela, la plupart ont changé pour utiliser un autre type de fonction d'activation, l'unité linéaire rectifiée, ou ReLU. Malgré son nom obtus, il est simplement défini comme $$R(z) = max(0, z)$$.
